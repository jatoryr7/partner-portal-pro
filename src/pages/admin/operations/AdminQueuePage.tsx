  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Campaign Queue</h2>
          <p className="text-muted-foreground">Status at a Glance • {campaigns.length} active campaigns</p>
        </div>
        <Badge variant="outline" className="text-muted-foreground rounded-none">
      ...
      <div className="grid grid-cols-5 gap-4 min-h-[calc(100vh-200px)]">
        {STAGE_ORDER.map((stage) => (
          <div key={stage} className="flex flex-col">
            {/* Column Header */}
            <div className="bg-card border border-border rounded-none p-3 sticky top-0 z-10">
              <div className="flex items-center justify-between">
            ...
            {/* Column Content */}
            <ScrollArea className="flex-1 bg-muted/30 border-x border-b border-border rounded-none p-2">
              <div className="space-y-2">
                {groupedCampaigns[stage].length === 0 ? (
                  ...
                  groupedCampaigns[stage].map((card, index) => (
                    ...
                    >
                      <Card 
                        className={`cursor-pointer hover:shadow-md transition-all border-l-4 rounded-none ${
                      ...
                      >
                        <CardContent className="p-3 space-y-3">
                          ...
                          <Badge 
                            variant="outline" 
                            className={`text-xs rounded-none ${PRIORITY_COLORS[card.priority].bg} ${PRIORITY_COLORS[card.priority].text}`}
                          >
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="h-7 w-7 text-purple-500 hover:text-purple-600 hover:bg-purple-50 rounded-none"
                                  onClick={(e) => {
                                    e.stopPropagation();
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="h-7 w-7 text-blue-500 hover:text-blue-600 hover:bg-blue-50 rounded-none"
                                  onClick={(e) => {
                                    e.stopPropagation();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage campaigns and inventory</p>
        </div>
      </div>

      {/* Tabs */}
      <Tabs defaultValue="campaign-queue" className="w-full">
        <TabsList className="rounded-none border border-border">
          <TabsTrigger value="campaign-queue" className="rounded-none">
            Campaign Queue
          </TabsTrigger>
          <TabsTrigger value="inventory-matrix" className="rounded-none">
            Inventory Matrix
          </TabsTrigger>
        </TabsList>

        <TabsContent value="campaign-queue" className="mt-6">
          <CampaignQueue />
        </TabsContent>

        <TabsContent value="inventory-matrix" className="mt-6">
          <InventoryMatrix />
        </TabsContent>
      </Tabs>
    </div>
